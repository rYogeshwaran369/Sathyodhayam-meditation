...
class Djabam extends Component {
  state = {
    dhiyanams: [],
    isLoading: true,
  ...
  handleDelete = async (id) => {
    try {
      await firestore.collection('jabam').doc(id).delete();
  ...
  handleSave = async () => {
    ...
    try {
      await firestore.collection('jabam').doc(selectedDhiyanam.id).update({
        title,
        imageUrl,
        songUrl,
        times,
  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        <Menubar />
        <TouchableOpacity style={styles.button} onPress={() => this.props.navigation.navigate('Ajabam')}>
            <Text style={styles.buttonText}>Add Jabam</Text>
          </TouchableOpacity>
